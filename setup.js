 ...
 *   npx github:BNSTECH/rn-fastlane-kit      (or: node setup.js during local dev)
 *
...
let _closed = false;
rl.on('line', (line) => {
  if (_pending) { const p = _pending; _pending = null; p(line); }
  else _lineQueue.push(line);
});
rl.on('close', () => {
  _closed = true;
  if (_pending) { const p = _pending; _pending = null; p(null); }
});
function nextLine() {
  if (_lineQueue.length) return Promise.resolve(_lineQueue.shift());
  if (_closed) return Promise.resolve(null);
  return new Promise((res) => { _pending = res; });
}
...
function detectIos() {
  ...
  let workspace = '', scheme = '', bundleId = '', teamId = '';
  try {
    ...
    if (projDir) {
      ...
      const bundles = [...pbx.matchAll(/PRODUCT_BUNDLE_IDENTIFIER\s*=\s*"?([^";]+)"?;/g)]
        .map((x) => x[1].trim())
        .filter((v) => v && !/test/i.test(v) && !v.includes('$('));
      if (bundles.length) bundleId = bundles[0];
      const team = pbx.match(/DEVELOPMENT_TEAM\s*=\s*"?([^";]+)"?;/);
      if (team) teamId = team[1].trim();
    }
  } catch { /* ios dir missing — leave blanks */ }
  return { workspace, scheme, bundleId, teamId };
}

function detectAppName() {
  try { return JSON.parse(readSafe(path.join(PROJECT_ROOT, 'package.json'))).name || ''; }
  catch { return ''; }
}
...
async function main() {
  ...
  const ANDROID_KEY_PASSWORD = await askSecret('Key password (gizli, boş = keystore ile aynı)')
    .then((v) => v || ANDROID_KEYSTORE_PASSWORD);
